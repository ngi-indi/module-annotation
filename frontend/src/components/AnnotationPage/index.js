...
const AnnotationPage = () => {
  ...
  const handleNext = (rowData) => {
    ...
    if(selectedValues.length === numberOfRows){
      handleSave();
      if (isLastPage()) {
        toast.success("Great job! You've successfully annotated all the sentences!",{
          position: "top-center"});
          setShowtable(false);
      }
      else{
        toast.success("Your annotations have been saved! You may proceed with annotating the remaining sentences",{
          position: "top-center"});
      ...
      toast.error("Please ensure all sentences are fully annotated",{
        position: "top-center"});
  ...
  const dropdownAnswerTemplate = (rowData) => {
    return (
      <Dropdown 
        ...
        placeholder="Please make your decision"
      />
    );
  };
    ...
    return(
      <div className='card'>
        ...
        >
          ...
          <Column field="lista_bias" header="Bias type" body={(rowData) => (
           <Tooltip title={tooltipTemplate(rowData)}>
           <span>{rowData.lista_bias}</span>
         </Tooltip>
        )} />
          <Column field="flag_bias" header="Your decision" body={dropdownAnswerTemplate}></Column>
        </DataTable>
...
//-----------------------------Return Principale----------------------------------
  ...
  return(<div>
    ...
    <div class="card m-5 d-block">
      {showtable &&(<h2 class="card-header">Review bulk no. {currentPage+1}</h2>)}
      {!showtable &&(<h2 class="card-header">Your contribution is enough for now</h2>)}
      <div class="card-body">

      {showtable && ( <VisualizzaFrasi2/>)}

      {!showtable && (<p>There are no new sentences available for review. Please wait, or consider updating your topics</p>)}
            
      </div>
    </div>
      <Col className="d-flex justify-content-around">
        <Button onClick={handleCancel} variant="secondary" className="mt-2 button-red">
          Back to Dashboard
        </Button>

        <Button onClick={handleNext} disabled={!showtable}variant="primary" className="mt-2 button-red">
          Save 
        </Button>
      </Col>
    </div>
